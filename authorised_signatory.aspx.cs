...
public partial class authorised_signatory : System.Web.UI.Page
{
    DAL d = new DAL();
    ...
    protected void btn_approve_Command(object sender, CommandEventArgs e)
    {
        ...
        string auth_sign_approval_date = now.ToString();
        string Rejection_Reason = "";

        int cash_v = d.operation("Update pay_cash_voucher set status=6, Rejection_Reason='" + Rejection_Reason + "',cash_login_id2 = '" + Session["login_id"].ToString() + "', auth_sign_approval_date = '" + auth_sign_approval_date + "' where id=" + id + "");
    ...
    protected void btn_reject_Command(object sender, CommandEventArgs e)
    {
        ...
        if (Rejection_Reason != "")
        {
            ...
            int cash_v = d.operation("Update pay_cash_voucher set status=7,Rejection_Reason='" + Rejection_Reason + "' where id=" + id + "");
            if (cash_v > 0)
            {
                cash_gv_code();
    ...
    public void cash_gv_code()
    {
        ...
        try
        {
            ...
            d.con.Open();
            MySqlDataAdapter adp_grid = new MySqlDataAdapter("select id, case when status = '0' then 'Waiting For Approval' when status = '1' then 'Record Approved Successfully' when status ='2' then 'Approved By Accounts' when status ='3' then 'Rejected By Accounts' when status ='4' then 'Approved By HOD'  when status ='5' then 'Rejected By HOD' when status ='6' then 'Approved By Authorised Signatory' when status=7 then 'Rejected By Authorised Signatory' when status=8 then 'Paid' end as 'status', cash_rs,receiver_name,debited_to,DATE_FORMAT(request_date, '%d/%m/%Y') AS 'request_date',narration,soft_copy_file, Rejection_Reason ,status from pay_cash_voucher where comp_code= '" + Session["comp_code"].ToString() + "' AND status in (4,6,7)", d.con);
    ...
    protected void gv_cash_voucher_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ((LinkButton)e.Row.Cells[8].FindControl("btn_approve")).Visible = true;
            ((LinkButton)e.Row.Cells[9].FindControl("btn_reject")).Visible = true;

        }
        catch { }
        if (e.Row.Cells[7].Text == "Approved By Authorised Signatory")
        {
            try
            {
                ((LinkButton)e.Row.Cells[8].FindControl("btn_approve")).Visible = false;
                ((LinkButton)e.Row.Cells[9].FindControl("btn_reject")).Visible = false;
            }
            catch { }
        }
        if (e.Row.Cells[7].Text == "Rejected By Authorised Signatory")
        {
            try
            {
                ((LinkButton)e.Row.Cells[8].FindControl("btn_approve")).Visible = false;
    ...
    protected void btn_tally_report_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            query = "SELECT cash_rs,receiver_name, debited_to, date_format(request_date,'%d/%m/%Y') as 'request_date',narration,payment_method,date_format(payment_date,'%d/%m/%Y') as 'payment_date',Rejection_Reason,case when status = '0' then 'Waiting For Approval' when status = '1' then 'Record Approved Successfully' when status ='2' then 'Approved By Accounts_Department' when status ='3' then 'Rejected By Account_Department' when status ='4' then 'Approved By HOD'  when status ='5' then 'Rejected By HOD' when status ='6' then 'Approved By Authorised Signatory' when status=7 then 'Rejected By Authorised Signatory' when status=8 then 'Paid' end as 'status' FROM pay_cash_voucher where status in(4,6,7)  ";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=Authorised_Signatory_Report" + ".xls");
                Response.Charset = "";
